...
describe(filterList.name, () => {
  describe('numeric comparisons', () => {
    ...
    it('should filter using equals operator', () => {
      const query: FilterItem<number> = { operator: '==', value: 3 };
  ...
  describe('string comparisons', () => {
    ...
    it('should filter strings using equals operator', () => {
      const query: FilterItem<string> = { operator: '==', value: 'banana' };
  ...
  describe('array operators', () => {
    ...
    it('should filter using in operator', () => {
      const query: FilterItem<number> = { operator: 'in', value: [2, 4] };
      expect(filterList(items, fieldName, query)).toEqual([items[1], items[3]]);
    });

    it('should filter using not-in operator', () => {
      const query: FilterItem<number> = { operator: 'not-in', value: [2, 4] };
      expect(filterList(items, fieldName, query)).toEqual([
        items[0],
        items[2],
        items[4],
    ...
    it('should throw error when in operator value is not an array', () => {
      const query: FilterItem<number> = { operator: 'in', value: 2 as any };
  ...
  describe('edge cases', () => {
    it('should handle empty array', () => {
      const query: FilterItem<number> = { operator: '==', value: 1 };
    ...
    it('should handle array with duplicate values', () => {
      ...
      const query: FilterItem<number> = { operator: '==', value: 2 };
