...
import { SubscriptionsRepository, UserCompanyRelationsRepository } from '../../../repositories';
import { PERMISSIONS_BY_ROLE, UserCompanyRole } from '../../entities';
...
export class AuthService {
  ...
  public async generateUserToken(userId: string, logger: ExecutionLogger): Promise<string> {
    ...
    const token = await admin.auth().createCustomToken(userId, this._getBaseClaims(userId)); // we only add basic claims because claims added in token creation cannot be later updated
  ...
  public async getUserPermissions(userId: string, logger: ExecutionLogger): Promise<UserPermissions> {
    ...
    const response: {app_user_id: string, companies: { [companyId: string]: string[] } } = {
      ...this._getBaseClaims(userId),
      companies: {}
    };
  ...
  private _getBaseClaims(userId: string): { app_user_id: string } {
    return {
      app_user_id: userId,
    };
  }
