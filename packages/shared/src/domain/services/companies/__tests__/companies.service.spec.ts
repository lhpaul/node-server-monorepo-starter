...
describe(CompaniesService.name, () => {
  ...
  describe(CompaniesService.prototype.getActiveSubscriptions.name, () => {
    ...
    it('should return active subscriptions for a company', async () => {
      ...
      mockSubscriptionsService.getResourcesList.mockResolvedValue(mockSubscriptions);
    ...
    it('should use current date for filtering active subscriptions', async () => {
      // Arrange
      const differentDate = new Date('2024-06-15T12:00:00Z');
      jest.useFakeTimers().setSystemTime(differentDate);
      ...
      expect(mockSubscriptionsService.getResourcesList).toHaveBeenCalledWith({
        companyId: [{ operator: '==', value: mockCompanyId }],
        startsAt: [{ operator: '<=', value: differentDate }],
        endsAt: [{ operator: '>=', value: differentDate }],
      }, mockLogger);
    });

    it('should handle subscriptions with exact start and end dates', async () => {
      // Arrange
      const exactDate = new Date('2024-01-15T12:00:00Z');
      jest.useFakeTimers().setSystemTime(exactDate);
      ...
      expect(mockSubscriptionsService.getResourcesList).toHaveBeenCalledWith({
        companyId: [{ operator: '==', value: mockCompanyId }],
        startsAt: [{ operator: '<=', value: exactDate }],
        endsAt: [{ operator: '>=', value: exactDate }],
      }, mockLogger);
    });

    it('should handle multiple subscriptions for the same company', async () => {
      // Arrange
      const mockSubscriptions = [
        new Subscription({
          id: 'sub-1',
    ...
    it('should handle service errors gracefully', async () => {
      ...
      mockSubscriptionsService.getResourcesList.mockRejectedValue(mockError);
    ...
    it('should verify correct query structure for active subscriptions', async () => {
      // Arrange
      mockSubscriptionsService.getResourcesList.mockResolvedValue([]);
      ...
      const actualCall = mockSubscriptionsService.getResourcesList.mock.calls[0][0] as any;
      expect(actualCall).toHaveProperty('companyId');
      expect(actualCall).toHaveProperty('startsAt');
      expect(actualCall).toHaveProperty('endsAt');
      expect(actualCall.companyId).toHaveLength(1);
      expect(actualCall.startsAt).toHaveLength(1);
      expect(actualCall.endsAt).toHaveLength(1);
      expect(actualCall.companyId[0]).toHaveProperty('operator', '==');
      expect(actualCall.companyId[0]).toHaveProperty('value', mockCompanyId);
      expect(actualCall.startsAt[0]).toHaveProperty('operator', '<=');
      expect(actualCall.startsAt[0]).toHaveProperty('value', mockNow);
      expect(actualCall.endsAt[0]).toHaveProperty('operator', '>=');
      expect(actualCall.endsAt[0]).toHaveProperty('value', mockNow);
    });
    ...
    it('should verify subscription object structure', async () => {
      // Arrange
      const mockSubscriptions = [
        new Subscription({
          id: 'sub-1',
      ...
      expect(result[0]).toHaveProperty('id', 'sub-1');
      expect(result[0]).toHaveProperty('companyId', mockCompanyId);
